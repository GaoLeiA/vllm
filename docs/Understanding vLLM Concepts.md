...
GQA单独讲下。单独讲下continuous batching例子。

### Planner Response

好的，让我分别详细讲解 **GQA** 和 **Continuous Batching**，并创建一个演示例子。

---

## 📘 Part 1: GQA (Grouped Query Attention) 详解
...
│   ├── Q heads: 32                                                       │
│   ├── K heads: 32                     每个 Q 有独立的 K, V              │
│   └── V heads: 32                     KV Cache: 100%                    │
│                                                                         │
│   MQA (Multi-Query Attention)         Google 2019                       │
│   ├── Q heads: 32                                                       │
│   ├── K heads: 1                      所有 Q 共享 1 个 K, V             │
│   └── V heads: 1                      KV Cache: 3.125% (1/32)           │
│                                       ⚠️ 精度损失较大                    │
│                                                                         │
│   GQA (Grouped Query Attention)       Google 2023                       │
│   ├── Q heads: 32                                                       │
│   ├── K heads: 8                      每 4 个 Q 共享 1 个 K, V          │
│   └── V heads: 8                      KV Cache: 25% (8/32)              │
│                                       ✅ 平衡精度和效率                  │
│                                                                         │
└─────────────────────────────────────────────────────────────────────────┘
```

### 1.2 GQA 数学原理 (以 Qwen3-4B 为例)
...
模型配置:
  ...
  head_dim = 128
  num_groups = 32 / 8 = 4       (每组 4 个 Q 共享 1 个 KV)
```
...
│                                          │
│  Q: [seq, 32, 128]                       │
│  K: [seq, 32, 128]  ← 32 个独立的 KV     │
│  V: [seq, 32, 128]                       │
│                                          │
│  每个 Q_i 只与对应的 K_i, V_i 计算       │
│                                          │
│  Q_0 ─── K_0, V_0                        │
│  Q_1 ─── K_1, V_1                        │
│  Q_2 ─── K_2, V_2                        │
...
│  Q_31 ── K_31, V_31                      │
│                                          │
│  KV Cache 大小: 32 × 128 × 2 = 8192      │
└──────────────────────────────────────────┘

┌─────────────── GQA (分组) ───────────────┐
│                                          │
│  Q: [seq, 32, 128]                       │
│  K: [seq, 8, 128]   ← 只有 8 个 KV       │
│  V: [seq, 8, 128]                        │
│                                          │
│  每 4 个 Q 共享 1 个 KV                  │
│                                          │
...
Q = X @ W_q  # [seq_len, 32 * 128] = [seq_len, 4096]
K = X @ W_k  # [seq_len, 8 * 128]  = [seq_len, 1024]  ← 更小!
V = X @ W_v  # [seq_len, 8 * 128]  = [seq_len, 1024]  ← 更小!

# Step 2: Reshape
Q = Q.view(seq_len, 32, 128)   # [seq, 32, 128]
K = K.view(seq_len, 8, 128)    # [seq, 8, 128]
V = V.view(seq_len, 8, 128)    # [seq, 8, 128]

# Step 3: 扩展 KV 以匹配 Q 的 head 数
# 关键操作: repeat_interleave
num_groups = 32 // 8  # = 4
K_expand = K.repeat_interleave(num_groups, dim=1)  # [seq, 8, 128] → [seq, 32, 128]
V_expand = V.repeat_interleave(num_groups, dim=1)  # [seq, 8, 128] → [seq, 32, 128]

# repeat_interleave 的效果:
# K: [K0, K1, K2, K3, K4, K5, K6, K7]
# K_expand: [K0, K0, K0, K0, K1, K1, K1, K1, K2, K2, K2, K2, ...]

# Step 4: 标准 Attention 计算
scores = torch.einsum("qhd,khd->hqk", Q, K_expand) / sqrt(128)
attn_weights = softmax(scores, dim=-1)
output = torch.einsum("hqk,khd->qhd", attn_weights, V_expand)
```

### 1.3 GQA 优势总结

| 指标 | MHA (32:32) | GQA (32:8) | 节省 |
|------|-------------|------------|------|
| **K Projection 参数** | 32 × 128 × 2560 | 8 × 128 × 2560 | **75%** |
| **V Projection 参数** | 32 × 128 × 2560 | 8 × 128 × 2560 | **75%** |
| **KV Cache (per token)** | 2 × 32 × 128 × 2 = 16 KB | 2 × 8 × 128 × 2 = 4 KB | **75%** |
| **模型精度** | 100% | ~99.5% | 几乎无损 |

---

## 📗 Part 2: Continuous Batching 详解
...
┌─────────────── Static Batching (传统) ───────────────────────────────┐
│                                                                       │
│  Request A: "Hello"        → 生成 200 tokens                         │
│  Request B: "Hi there"     → 生成 50 tokens                          │
│  Request C: "Hey"          → 生成 100 tokens                         │
│                                                                       │
│  时间线:                                                              │
│  ┌─────────────────────────────────────────────────────────────────┐ │
│  │ Step 1  │ Step 50 │ Step 100 │ Step 200                        │ │
│  ├─────────┼─────────┼──────────┼─────────────────────────────────┤ │
│  │ A B C   │ A B C   │ A _ C    │ A _ _      ← B,C 完成但 GPU 空闲 │ │
│  │ (计算)  │ (计算)  │ (浪费!)  │ (浪费!)                          │ │
│  └─────────┴─────────┴──────────┴─────────────────────────────────┘ │
│                                                                       │
│  ⚠️ 问题:                                                             │
│  1. 短请求完成后，GPU 资源闲置                                         │
│  2. 新请求必须等待整个 batch 完成                                      │
│  3. 响应延迟 = max(所有请求的生成时间)                                 │
│                                                                       │
└───────────────────────────────────────────────────────────────────────┘
```
...
│  时间线:                                                              │
│  ┌─────────────────────────────────────────────────────────────────┐ │
│  │ Step 1  │ Step 50 │ Step 51  │ Step 100 │ Step 101 │ Step 200  │ │
│  ├─────────┼─────────┼──────────┼──────────┼──────────┼───────────┤ │
│  │ A B C   │ A B C   │ A D C    │ A D E    │ A F E    │ A F G     │ │
│  │ (满载)  │ B完成↗  │ D插入↗   │ C完成,E入│ D完成,F入│ (持续满载)│ │
│  └─────────┴─────────┴──────────┴──────────┴──────────┴───────────┘ │
│                                                                       │
│  ✅ 优势:                                                             │
│  1. 完成的请求立即释放资源                                             │
...
### 2.3 Continuous Batching 代码示例

让我创建一个演示 Continuous Batching 的完整例子：

*Edited relevant file*

*User accepted the command `python examples/continuous_batching_demo.py`*

### Planner Response

演示成功运行！现在让我为你总结一下完整的内容：

---

## 📚 GQA + Continuous Batching 总结

### Part 1: GQA (Grouped Query Attention)
...
├────────────────────────────────────────────────────────────────────┤
│                                                                    │
│  传统 MHA:  每个 Q head 有独立的 K, V                               │
│            Q: 32 heads,  K: 32 heads,  V: 32 heads                 │
│                                                                    │
│  GQA:      多个 Q heads 共享同一组 K, V                            │
│            Q: 32 heads,  K: 8 heads,   V: 8 heads                  │
│            → 每 4 个 Q 共享 1 个 KV                                 │
│                                                                    │
│  计算方式:                                                         │
│    1. K, V 通过 repeat_interleave 扩展到 32 heads                  │
│    2. 然后进行标准的 attention 计算                                 │
│                                                                    │
│  优势:                                                             │
│    ✅ KV Cache 内存减少 75%                                        │
│    ✅ KV Projection 计算减少 75%                                   │
│    ✅ 精度几乎无损 (~99.5%)                                        │
│                                                                    │
└────────────────────────────────────────────────────────────────────┘
```
...
│                                                                    │
│  vLLM 调度策略:                                                     │
│    1. 每个 step，调度 running requests (decode)                     │
│    2. 如果有空余容量，从 waiting queue 取新请求 (prefill)            │
...
```
d:\projects\vllm\examples\continuous_batching_demo.py  # Continuous Batching 演示
d:\projects\vllm\examples\prefill_decode_demo.py       # Prefill/Decode 演示
```

这两个概念 (GQA + Continuous Batching) 是 vLLM 高效推理的核心技术！